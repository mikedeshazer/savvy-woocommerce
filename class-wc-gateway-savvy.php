...
add_action('init', 'savvy_shortcodes_init');
add_action('init', 'create_savvy_post_type');

add_filter('template_include', 'savvy_payment_page_template');

function savvy_payment_page_template($page_template)
{
    if (get_post_type() && get_post_type() === 'savvy_payment') {
...
function savvy_shortcodes_init()
{
    add_shortcode('savvy_payment_widget', 'savvy_payment_widget_shortcode');
}

function create_savvy_post_type()
{
    register_post_type('savvy_payment',
        array(
            'labels' => array(
                'name' => __('Savvy Payment'),
                'singular_name' => __('Savvy Payment')
            ),
            'public' => true,
            'has_archive' => false,
...
function savvy_payment_widget_shortcode($atts = array(), $content = null, $tag = '')
{
    wp_enqueue_style('savvy-css', plugin_dir_url(__FILE__) . 'assets/form/savvy.css');
    wp_enqueue_script('savvy-js', plugin_dir_url(__FILE__) . 'assets/form/savvy.js', array('jquery'));
    wp_enqueue_script('savvy-widget-js', plugin_dir_url(__FILE__) . 'assets/form/widget.js', array('savvy-js'));
    ...
    $gateway = WC_Gateway_Savvy::get_instance();
    $to_pay = 0;
    $total_paid = 0;
    $total_paid_fiat = 0;
...
function savvy_gateway_load()
{
     ...
     */
    add_filter('woocommerce_payment_gateways', 'woocommerce_savvy_add_gateway');

    function woocommerce_savvy_add_gateway($methods)
    {
        if (!in_array('WC_Gateway_Savvy', $methods, true)) {
            $methods[] = WC_Gateway_Savvy::get_instance();
        }
    ...
    class WC_Gateway_Savvy extends WC_Payment_Gateway
    {
         ...
         *
         * @return WC_Gateway_Savvy The *Singleton* instance.
         */
        ...
        protected function __construct()
        {
            ...
            $this->method_title = __('Savvy Payment', 'woocommerce');
            $this->method_description = __('Allows to accept crypto payments such as Bitcoin (BTC) and Ethereum (ETH)', 'woocommerce');
        ...
        public function init()
        {
            ...
            add_filter('plugin_action_links_savvy-crypto-payment-gateway-for-woocommerce/class-wc-gateway-savvy.php', array($this, 'plugin_action_links'));


            // Payment listener/API hook
            add_action('woocommerce_api_wc_gateway_savvy', array($this, 'check_ipn_response'));

            //Disable savvy if all Cryptocurrencies are disabled
        ...
        {
            if (isset($available_gateways['savvy']) && !$this->is_available()) {
                unset($available_gateways['savvy']);
        ...
        public function get_setting_link()
        {
            ...
            $section_slug = $use_id_as_section ? 'savvy' : strtolower('WC_Gateway_Savvy');
        ...
        {
            return add_query_arg(array('wc-api' => 'WC_Gateway_Savvy', 'address' => $token, 'order_id' => $order_id), home_url('/'));
        }

        public function get_ipn_link($order_id)
        {
            return add_query_arg(array('wc-api' => 'WC_Gateway_Savvy', 'order_id' => $order_id), home_url('/'));
        }

        public function get_status_link($id)
        {
            return add_query_arg(array('wc-api' => 'WC_Gateway_Savvy', 'status' => $id), home_url('/'));
        }
        ...
        public function process_admin_notices()
        {
            ...
            if (!$this->get_exchange_rate('eth')) {
                echo '<div class="error"><p>' . sprintf(__('Savvy plugin could not load the conversion rates, please check if your currency (%1$s) is supported.', 'woocommerce'), get_woocommerce_currency()) . '</p></div>';
            }

            //check API keys
            if (!$this->get_option('api_secret') || substr($this->get_option('api_secret'), 0, 3) != 'sec') {
                echo '<div class="error"><p>' . sprintf(__('Please set your API keys in <a href="%1$s">Savvy Settings</a>', 'woocommerce'), $this->get_setting_link()) . '</p></div>';
            }
        }
        ...
        public function init_currencies()
        {
            ...
            if (($this->get_option('testnet') == 'yes'))
                $url = $this->api_domain() . sprintf("/v3/currencies?token=%s", $this->get_option('api_testnet'));
...
invoice only. 86400 seconds by default. Set ‑1 for infinite lock", 'woocommerce'),
        ...
        function process_payment($order_id)
        {
            ...
            $order = wc_get_order($order_id);
            $savvy_page_id = $this->get_option('payment_page_id');
            $redirect = add_query_arg(array('order_id' => $order->get_id(), 'key' => $order->get_order_key()), get_permalink($savvy_page_id));
            ...
            if ($invoice == get_post_meta($order_id, $token . ' invoice', true)) {
                ...
                if ($isNewPayment) {
                    if (time() > $deadline) { //rate changed. recalculate crypto total?
                        self::log("Savvy IPN: late payment [" . $order_id . "]");
                        self::log("Savvy IPN: old total [" . $toPay . "]");
                        ...
                        if ($rate && $fiatTotal > 0) {
                            ...
                            if (true || $newCryptoTotal > $toPay) {
                                ...
                                self::log("Savvy IPN: new total [" . $toPay . "]");
                            }
                        }
                    }
                ...
                if ($params->confirmations >= $maxConfirmations) { //enough confirmations for this payment
                    ...
                    self::log("Savvy IPN: toPay [" . $toPay . "]");
                    self::log("Savvy IPN: paid [" . $amountPaid . "]");
                    self::log("Savvy IPN: total paid [" . $totalPaid . "]");
                    self::log("Savvy IPN: maxDifference [" . round($maxDifference, 9) . "]");
...
This timer is setup to lock in a fixed rate for your payment. Once it expires, rates may change.', 'woocommerce'),
                        ...
                        self::log("Savvy IPN: underpayment [" . $order_id . "]");
                        //if (!empty($this->debug_email)) { mail($this->debug_email, "savvy IPN: wrong amount [" . $order_id . "]", print_r($params, true)); }
...
This timer is setup to lock in a fixed rate for your payment. Once it expires, rates may change.', 'woocommerce'),
                ...
                self::log("Savvy IPN: wrong invoice [" . $order_id . ' / ' . get_post_meta($order_id, $token . ' invoice', true) . "]");
                if (!empty($this->debug_email)) {
                    mail($this->debug_email, "Savvy IPN: wrong invoice [" . $invoice . "]", print_r($params, true));
                }
            }
        ...
        function check_ipn_response()
        {
            ...
            self::log("Savvy IPN:" . print_r($_GET, true) . ' / ' . print_r($_POST, true));

            if (isset($_GET['order_id'])) {
                $order_id = intval($_GET['order_id']);
                $data = file_get_contents('php://input');
                self::log("Savvy IPN data:" . $data);
                if ($data) {
                ...
                wp_die("Savvy IPN Request Failure");
            }
        }
        ...
        public function get_address($token, $order_id, $total, $lock_address_timeout)
        {
            ...
            if (($this->get_option('testnet') == 'yes')) {
                $testnet = $this->get_option('api_testnet');
                $url = sprintf($this->api_domain() . '/v3/%s/payment/%s?token=%s&lock_address_timeout=%s', $token, urlencode($callbackUrl), $testnet, $lock_address_timeout);
            }

            self::log("savvy address request: " . $url);
            if ($contents = wp_remote_fopen($url)) {
                $response = json_decode($contents);
                self::log("savvy address response: " . print_r($response, true));
                if (isset($response->data->address)) {
                    ...
                    self::log("savvy API responded: " . $contents);
                }
            }
        }
        ...
        public static function plugin_activation()
        {
            ...
            if ($page_id && get_post_type($page_id) != 'savvy_payment') {
                wp_delete_post($page_id);
            }

            if (!$page_id) {
                add_action('init', function () use ($self) {
                    $guid = home_url('/savvy_payment/savvy');

                    $page_id = $self::get_post_id_from_guid($guid);
                    if (!$page_id) {
                        $page_data = array(
                            'post_status' => 'publish',
                            'post_type' => 'savvy_payment',
                            // 'post_author'    => 1,
                            // 'post_name'      => 'savvy',
                            'post_title' => 'savvy',
                            'post_content' => '[savvy_payment_widget]',
    ...
    class WC_Savvy extends WC_Gateway_Savvy
    {
        public function __construct()
        {
            _deprecated_function('WC_Savvy', '1.4', 'WC_Gateway_Savvy');
            parent::__construct();
        }
    }

    $GLOBALS['wc_savvy'] = WC_Gateway_Savvy::get_instance();
}

register_activation_hook(__FILE__, function () {

});
register_deactivation_hook(__FILE__, array('WC_Gateway_Savvy', 'plugin_deactivation'));
